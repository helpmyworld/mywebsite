...
tinymce.PluginManager.add('bootstrap', function(editor, url) {
    ...
    function getSelectedNode(callingFunction)
    {
        ...
        if(callingFunction == 'renderContent' && !jQuery(selectedNode).is('th') && !jQuery(selectedNode).is('td')) {
            return selectedNode;
        }
            ...
            if(parents.length > 0) {
                ...
                if(e.altKey === true) {
                    ...
                    for (var i = 0; i < specificParents.length; i++) {
                        ...
                        if($(currentNode).closest(bsElement)[0]) {
                            $(parents).each(function() {
                                if($(this).is(bsElement)) {
                    ...
                    if(specificParentFound === false) {
                        $(currentNode).after(newParagraph);
                    }
