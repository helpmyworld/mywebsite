...
class AdminController extends Controller
{
    ...
    public function register(Request $request){
        if($request->isMethod('post')){
            ...
            $adminsCount = Admin::where('username',$data['username'])->count();
            if($adminsCount>0){
...
//                });
                ...
                return redirect()->back()->with('flash_message_success','Admin Registered!');
    ...
    public function login(Request $request){
    	if($request->isMethod('post')){
    		$data = $request->all();
            $admin = Admin::where(['username' => $data['username'],'status'=>1])->first();
            ...
            if($admin != null){
                ...
                return redirect('/admin')->with('flash_message_error','Invalid Password');

        	}else{
                //echo "failed"; die;
                return redirect('/admin')->with('flash_message_error','Invalid Username');
        	}
    	}
...
//            echo "<pre>";print_r($data); die;
            ...
            if($adminCount>0){
                return redirect()->back()->with('flash_message_error', 'Admin Username already exist
                ...
                if ($data['type'] == "Admin") {
                    ...
                    $admin->save();
                    return redirect()->back()->with('flash_message_success', 'Admin Username successfully
                 added!');
...
//        $adminDetails = json_encode(json_encode($adminDetails));
//        echo "<pre>"; print_r($adminDetails); die;
        if($request->isMethod('post')){
            $data = $request->all();
//            echo "<pre>"; print_r($data); die;
            ...
            if ($data['type'] == "Admin") {
                Admin::where('username',$data['username'])->update(['password'=>bcrypt($data['password']),'status'=>$data['status']]);
                ...
                return redirect()->back()->with('flash_message_success', 'Admin Username successfully Updated!');
            }
        }
